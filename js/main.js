...
w2v.loadModel( './data/glove.6B.50d.txt', function(err, model){
    var output = model.mostSimilar( 'treatise', 20 );
    var output2 = model.getNearestWords( model.getVector( 'treatise' ), 3 )
    console.log(output);
    console.log(output2);
});
...
var commonList = _.slice(stopwords, [start = 0], [end = numOfCommon + 1]);
